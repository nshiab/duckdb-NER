...
struct ner_ctx * ner_load_from_file(const char *fname) {
    ...
    if (magic != 0x67676d6c) return nullptr;
    ...
    while (true) {
        ...
        for (int i = 0; i < n_dims; i++) {
            int32_t ne_cur; fin.read((char *)&ne_cur, sizeof(ne_cur)); ne[i] = ne_cur;
        }
...
void ner_eval(struct ner_ctx * ctx, int32_t n_threads, ner_vocab_id * tokens, int32_t n_tokens, float * logits) {
    ...
    struct ggml_init_params params = { .mem_size = ctx->buf_compute.size, .mem_buffer = ctx->buf_compute.data, .no_alloc = false };
    ...
    // embd norm
    {
        ...
        inpL = ggml_add(ctx0, ggml_mul(ctx0, ggml_repeat(ctx0, model.ln_e_w, inpL), inpL), ggml_repeat(ctx0, model.ln_e_b, inpL));
    }
    ...
    for (int il = 0; il < n_layer; il++) {
        ...
        // self-attention
        {
            struct ggml_tensor * Qcur = ggml_add(ctx0, ggml_repeat(ctx0, model.layers[il].q_b, cur), ggml_mul_mat(ctx0, model.layers[il].q_w, cur));
            struct ggml_tensor * Q = ggml_permute(ctx0, ggml_reshape_3d(ctx0, Qcur, d_head, n_head, N), 0, 2, 1, 3);

            struct ggml_tensor * Kcur = ggml_add(ctx0, ggml_repeat(ctx0, model.layers[il].k_b, cur), ggml_mul_mat(ctx0, model.layers[il].k_w, cur));
            struct ggml_tensor * K = ggml_permute(ctx0, ggml_reshape_3d(ctx0, Kcur, d_head, n_head, N), 0, 2, 1, 3);

            struct ggml_tensor * Vcur = ggml_add(ctx0, ggml_repeat(ctx0, model.layers[il].v_b, cur), ggml_mul_mat(ctx0, model.layers[il].v_w, cur));
        ...
        // attention norm
        {
            ...
            cur = ggml_add(ctx0, ggml_mul(ctx0, ggml_repeat(ctx0, model.layers[il].ln_att_w, cur), cur), ggml_repeat(ctx0, model.layers[il].ln_att_b, cur));
        }
        ...
        {
            cur = ggml_norm(ctx0, cur);
            cur = ggml_add(ctx0, ggml_mul(ctx0, ggml_repeat(ctx0, model.layers[il].ln_out_w, cur), cur), ggml_repeat(ctx0, model.layers[il].ln_out_b, cur));
        }
    ...
    struct ggml_tensor * res = ggml_add(ctx0, ggml_mul_mat(ctx0, model.classifier_weight, inpL), ggml_repeat(ctx0, model.classifier_bias, inpL));
...
int32_t ner_n_embd(struct ner_ctx * ctx) { return ctx->model.hparams.n_embd; }
int32_t ner_n_max_tokens(struct ner_ctx * ctx) { return ctx->model.hparams.n_max_tokens; }
int32_t ner_n_labels(struct ner_ctx * ctx) { return ctx->model.hparams.n_labels; }
const char* ner_vocab_id_to_token(struct ner_ctx * ctx, ner_vocab_id id) {
    auto it = ctx->vocab._id_to_token.find(id);
    if (it != ctx->vocab._id_to_token.end()) return it->second.c_str();
    it = ctx->vocab._id_to_subword_token.find(id);
    if (it != ctx->vocab._id_to_subword_token.end()) return it->second.c_str();
    return "[UNK]";
}
